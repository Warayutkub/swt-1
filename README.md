...
สองคำสั่งนี้ทำให้**เห็นผลรันสีแดงจริง ๆ** ได้ทันที

### 1. สาธิตอัตโนมัติ — ทำให้พังทีละแบบแล้วคืนสภาพให้เอง

```bash
python scripts/demo_failures.py
```

แก้โค้ดให้ผิด 5 แบบ รัน pytest ให้ดูว่าแดงตัวไหน แล้ว**คืนสภาพไฟล์ให้เสมอ**
แม้จะกด Ctrl-C กลางทาง — ตรวจด้วย `git status` ได้ว่าไม่มีอะไรค้าง

```bash
python scripts/demo_failures.py --list    # ดูรายการกรณี
python scripts/demo_failures.py 2         # รันเฉพาะกรณีที่ 2
python scripts/demo_failures.py --full    # เอา traceback เต็ม
python scripts/demo_failures.py --restore # คืนสภาพฉุกเฉิน
```

### 2. บังคับให้เทสต์ที่ "รู้ว่าพัง" รายงานเป็นสีแดง

```bash
python -m pytest --runxfail
```

`--runxfail` สั่งให้ pytest เลิกให้อภัยเทสต์ที่ติดป้าย `xfail`
TC-P17 (ฟีเจอร์ที่ยังไม่ได้ทำ) จะขึ้นเป็น `FAILED` เต็มรูปแบบพร้อม traceback
...
### สิ่งที่แต่ละกรณีทำให้แดง (วัดมาแล้วทุกแถว)
